...
void start_monitoring(const char *csv_path){
    ...
    while (keepRunning==1) {
        ...
        for(short i=0;i<active_gpu_count;i++){
            fprintf(ctx.fp,"%d,%ld,%.2f,%lld,%lld,%lld,%lld\n",ctx.entityId[i],ctx.timestamp[i],ctx.power[i],ctx.gpu_temp[i],ctx.gpu_frame_buffer_used[i],ctx.sm_clock_freq[i],ctx.pcie_replay_cnt[i]);
        }
        fflush(fp);
        sleep(1);
    }
